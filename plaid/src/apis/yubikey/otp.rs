...
impl Yubikey {
    pub async fn verify_otp(&self, otp: &str, _: &str) -> Result<String, ApiError> {
        ...
        let nonce: [u8; 16] = rand::generate(&self.rng).map_err(|_| ApiError::YubikeyError(YubikeyError::RandError))?.expose();
        ...
        let data = res.text().await.map_err(|_| ApiError::YubikeyError(YubikeyError::NoData))?;
        ...
        let signature = response_items.remove("h").ok_or(ApiError::YubikeyError(YubikeyError::NoSignature))?;
        ...
        if let Err(_) = hmac::verify(
            ...
            &base64::decode(signature).map_err(|_| ApiError::YubikeyError(YubikeyError::BadData))?,
        ) {
        ...
        let status = response_items.get("status").ok_or(ApiError::YubikeyError(YubikeyError::NoStatus))?;
